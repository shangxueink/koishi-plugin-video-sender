...
var Commander = class {
  constructor(ctx, config = {}) {
    ...
    ctx.on("bot-status-updated", async (bot) => {
      if (bot.status !== Universal3.Status.ONLINE || !bot.updateCommands) return;
  ...
  defineElementDomain(name, key = name, type = name) {
    this.domain(name, (source, session) => {
      const code = h3.from(source, { type });
...
var KoishiSession = class {
  ...
  async send(fragment, options = {}) {
    const elements = h5.normalize(fragment);
  ...
  async withScope(scope, callback) {
    ...
    try {
      ...
      return h5.transform(result, {
        i18n: /* @__PURE__ */ __name((params, children) => h5.i18n({
          ...params,
  ...
  async execute(argv, next) {
    ...
    return this.withScope(`commands.${command.name}.messages`, async () => {
      ...
      if (!shouldEmit) return h5.normalize(result);
  ...
  prompt(...args) {
    ...
    return new Promise((resolve) => {
      const dispose = this.middleware(async (session, next) => {
        ...
        resolve(value);
        if (isNullable4(value)) return next();
      });
