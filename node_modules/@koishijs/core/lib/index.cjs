...
__reExport(index_exports, require("@koishijs/utils"), module.exports);
__reExport(index_exports, require("minato"), module.exports);
...
var Commander = class {
  ...
  defineElementDomain(name, key = name, type = name) {
    this.domain(name, (source, session) => {
      const code = import_core5.h.from(source, { type });
...
var KoishiSession = class {
  ...
  resolve(source, ...params) {
    ...
    if (!(0, import_minato.isEvalExpr)(source)) return source;
  ...
  async send(fragment, options = {}) {
    const elements = import_core7.h.normalize(fragment);
    if (!elements.length) return;
    options.session = this;
    return this.bot.sendMessage(this.channelId, elements, this.event.referrer, options).catch((error) => {
      logger3.warn(error);
      return [];
    });
  }
  ...
  async _observeChannelLike(channelId, fields = []) {
    ...
    const cache = (0, import_utils4.observe)(data, async (diff) => {
  ...
  async withScope(scope, callback) {
    ...
    try {
      ...
      return import_core7.h.transform(result, {
        i18n: /* @__PURE__ */ __name((params, children) => import_core7.h.i18n({
          ...params,
  ...
  async execute(argv, next) {
    ...
    return this.withScope(`commands.${command.name}.messages`, async () => {
      ...
      if (!shouldEmit) return import_core7.h.normalize(result);
  ...
  prompt(...args) {
    ...
    return new Promise((resolve) => {
      const dispose = this.middleware(async (session, next) => {
        ...
        resolve(value);
        if ((0, import_cosmokit8.isNullable)(value)) return next();
      });
...
var Permissions = class {
  constructor(ctx) {
    ...
    this.define("authority:(value)", {
      check: /* @__PURE__ */ __name(({ value }, { user }) => {
        return !user || user.authority >= +value;
      }, "check"),
      list: /* @__PURE__ */ __name(() => Array(5).fill(0).map((_, i) => `authority:${i}`), "list")
    });
...
var KoishiBot = class {
  ...
  async broadcast(channels, content, delay = this.ctx.root.config.delay.broadcast) {
    ...
    for (let index = 0; index < channels.length; index++) {
      if (index && delay) await (0, import_utils5.sleep)(delay);
      try {
